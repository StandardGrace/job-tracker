# RatTracer — Technical Design

This is the technical design and engineering decision log for RatTracer — architecture, API, schema, roadmap, and the reasoning behind choices made along the way. For a plain-language overview of what the app does, see the [main README](../README.md).
...
- ✅ **Phase 5 — Documents (MVP scope):** `folderLink` on each application, a detail modal (`@angular/cdk/dialog`) showing the status history timeline and a folder-link button, and backend logic so `PUT` actually appends to `statusHistory` when a drag-and-drop status change happens. The `documents` FormArray (individually-labeled résumé/cover-letter/job-posting links) was descoped from this phase — the shared Drive folder link already covers that need for now; moved to its own backlog ticket for a post-MVP pass.
...
- ✅ **Editable dates & correctable history:** `dateApplied` is now an editable field in the create/edit form (previously write-once at creation, requiring a manual MongoDB edit to backdate). Editing `dateApplied` also keeps the seed `statusHistory` entry in sync, so the timeline doesn't silently drift from the corrected date. The status history timeline itself is now editable from the detail modal — individual entries' dates can be corrected and entries can be deleted outright, covering the case where a drag-and-drop mistake (wrong card moved) needs to be cleaned out of the record. All date rendering (`dateApplied`, history entries) is explicitly formatted in UTC to avoid the classic off-by-one-day bug that shows up when a UTC-midnight date is rendered in a timezone behind UTC.
...
`PUT` applies whatever fields are present in the request body via Mongoose's `.set()`, including whole-array replacement of `statusHistory` — this is what the in-app date/history editing relies on, without needing dedicated subresource routes.
...
`ng serve` proxies `/api` requests to `http://localhost:3000` via `job-tracker/proxy.conf.json` (wired into `angular.json`'s `development` serve config) — without it, the Angular dev server has no way to reach the Express API, since the production nginx reverse-proxy that normally handles this doesn't exist in local dev.
...
Create/edit and the card detail view (status history + folder link) both now happen through modals (`@angular/cdk/dialog`) — see Engineering Decisions for the architecture. Not yet built:
...
- **Local dev proxy (`job-tracker/proxy.conf.json`) added to fix a dev-only gap.** `ApplicationData`'s relative `/api` path works in production because nginx reverse-proxies it, but `ng serve` has no equivalent — a bare `ng serve` was hitting `localhost:4200/api/applications`, which doesn't exist, and failing with "could not load applications." Standard Angular CLI dev-proxy config, wired into `angular.json`'s `development` serve configuration, so `ng serve` now forwards `/api` to `localhost:3000` the same way nginx does in production.
- **`dateApplied` editability and the `statusHistory[0]` sync fix.** The create/edit form originally had no `dateApplied` field at all, so every application permanently kept its creation-time date — the only way to backdate one was a manual MongoDB edit. Added the field to the form (defaults to today on create, patched from the existing value on edit). This surfaced a follow-on bug: `POST` seeds `statusHistory[0]` from `dateApplied` at creation time, but editing `dateApplied` later via `PUT` didn't touch that seed entry, so the two silently drifted apart (the card would show a corrected date while the "Applied" history entry stayed frozen at the original creation date). Fixed in the `PUT` route by detecting an actual `dateApplied` change (compared before `.set()` overwrites it, same pattern as the existing `statusChanged` check) and syncing `statusHistory[0].date` to match. Deliberately scoped to index `0` specifically — that's always the original seed entry, since nothing else in the app removes or reorders the array — rather than searching by `status === 'Applied'`, which could be ambiguous if a card were ever moved back to Applied after leaving it.
- **UTC-explicit date rendering to avoid an off-by-one-day display bug.** HTML `<input type="date">` values (and the corresponding stored `Date` objects) represent UTC midnight for a given calendar day. Angular's `date` pipe defaults to the browser's local timezone when no timezone argument is given, so a date typed as `2026-07-01` was rendering as `Jun 30, 2026` for anyone in a timezone behind UTC — the underlying data was correct, only the display was wrong. Fixed by passing `'UTC'` explicitly as the third argument to every `date` pipe usage (`application-board.html`, `application-detail.html`), so calendar dates render consistently regardless of viewer timezone.
- **Status history made directly editable/deletable from the detail modal, no backend route changes needed.** The trigger case: dragging the wrong card by accident, noticing, and dragging it back — leaving two spurious `statusHistory` entries with no way to remove them (short of a manual MongoDB edit, which was found to delete the whole document rather than a single array entry when attempted directly). Since `PUT` already replaces whatever fields are present in the request body via `.set()`, including whole-array replacement, no new subresource routes were needed. `ApplicationDetail` now builds a `FormGroup` wrapping a `FormArray` of `{ status, date }` groups (one per history entry, status shown read-only, date editable), with a delete button per row removing that entry from the `FormArray`. "Save Changes" sends the trimmed/edited array back via the existing `applicationData.update()` call. Deliberately scoped to date-only editing, not status — correcting a wrong date or removing a bad entry covers the actual failure mode; letting the status text itself be freely edited would risk corrupting the record with values outside the app's own enum with no real benefit.
